# %% [markdown]
# # Paired Random Forest vs TabPFN corrected standard errors
#
# Computes paired Nadeau-Bengio corrected standard errors for fold-level
# performance differences between Random Forest and TabPFN.
...
MODEL_ORDER = ["Random Forest", "TabPFN"]
...
for group_values, group in paired_rows.groupby(GROUP_COLUMNS, sort=True):
    ...
    for metric in METRICS:
        ...
        if metric in ["rmse", "mae"]:
            differences = aligned[BASELINE_MODEL] - aligned[TARGET_MODEL]
        else:
            differences = aligned[TARGET_MODEL] - aligned[BASELINE_MODEL]
...
summary_path = RESULTS_DIR / "random_forest_tabpfn_paired_corrected_se_differences.csv"
full_sample_path = (
    RESULTS_DIR / "random_forest_tabpfn_full_sample_paired_corrected_se_differences.csv"
)
